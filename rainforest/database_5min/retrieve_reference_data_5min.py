...
class Updater(object):
        ...
        for prod in products:
            try:
                if prod == 'CPC' or prod == 'CPCH':
                    files = retrieve_prod(start_time, end_time, prod, 
                                            pattern = '*5.801.gif',
    ...
    def process_all_timesteps(self):
        ...
        for i, tstep in enumerate(all_timesteps):
            logging.info(f"Processing timestep {str(tstep)} ({(100*i/len(all_timesteps)):.2f}%) \n")
            ...
            if hour_of_year != current_hour:
                ...
                if include_cpccv:
                    logging.info('Retrieving product CPC.CV {}'.format(current_hour))

                    # CPC.CV only contains a measurement every full hour
                    tstep_xls = datetime.datetime.strptime(current_hour,'%Y%m%d%H')    
                    cpccv, cpc_xls, aqc_xls = retrieve_CPCCV(tstep_xls, stations_to_get)
                    
                    # Replace NAN
                    cpccv[np.isnan(cpccv)] = fill_value
                    cpc_xls[np.isnan(cpc_xls)] = fill_value
                    aqc_xls[np.isnan(aqc_xls)] = fill_value

                    # Assign CPC.CV values to rows corresponding to nx = ny = 0
                    data_cpccv.extend(cpccv)
                    data_cpcxls.extend(cpc_xls)
                    data_aqcxls.extend(aqc_xls)
            ...
            for j, prod in enumerate(self.products):
                ...
                if 'MV' in prod:
                    ...
                    data_prod = np.zeros((N), dtype = np.float32) + np.nan
                    try:
                        proddata = read_cart(files[-1]) # get latest file
                    except:
                        # fill with missing values, we don't care about the exact dimension
                    ...
                    for sta in stations_to_get: # Loop on stations
                        for nx in self.neighb_x:
                            for ny in self.neighb_y:
                                ...
                                # Get idx of Cart pixel in 2D map
                                idx = lut_cart[sta][strnb]
                                data_prod[idx_row] = proddata[idx[0],idx[1]]
                                # Add next row
                                idx_row += 1
            ...
            data_stations.extend(data_allprod)
            ...
            if save_output and len(data_cst):
                ...
                data_stations = np.array(data_stations)
                data_cst = np.array(data_cst)
                # Concatenate metadata and product data
                all_data = np.hstack((data_cst, data_stations))
                dic = OrderedDict()
                ...
                if include_cpccv:
                    ...
                    all_data_cpccv = np.hstack((data_cst_cpccv, data_cpccv, data_cpcxls, data_aqcxls))
