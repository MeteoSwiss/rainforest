...
#                    self.summaries[table].to_csv(summary_file, index=False)
    ...
    def update_reference_data(self, gauge_table_name,  output_folder, 
        ...
        all_stations = tab['STATION'].to_numpy()
        ...
        days_per_job = max([1, len(days_to_process)//num_jobs + 1])
        ...
        # Get all stations for every unique time
        station_dict = defaultdict(list)
        for i, group_idx in enumerate(idx):
            station_dict[group_idx].append(all_stations[i])
        station_lines = ['{:d},{}\n'.format(int(unique_times[i]), ','.join(station_dict[i]))
        ...
        cwd = os.path.dirname(os.path.realpath(__file__))
        # Create slurm files
        max_simul_jobs = config_r['MAX_SIMULTANEOUS_JOBS']
        tf = tmp_folder + 'task_file_reference_${SLURM_ARRAY_TASK_ID}'

        # Create job file
        fname = tmp_folder + '/getdata_reference_{:d}_{:d}.job'.format(0, len(task_files)-1)
        logging.info(f"Writing job file {fname}")
        file = open(fname,'w')
        file.write(self.config['SLURM_HEADER'].format('ref','ref','REF'))
        file.write('#SBATCH --array={:d}-{:d}%{:d}\n\n'.format(0,len(task_files), max_simul_jobs))
        file.write(self.config['PYTHON_HEADER'])
        ...
        logging.info('Submitting job {}'.format(fname))
        subprocess.call('sbatch {:s}'.format(fname), shell = True)
    ...
    def update_radar_data(self, gauge_table_name,  output_folder,
        ...
        # Create job file
        fname = tmp_folder + '/getdata_radar_{:d}_{:d}.job'.format(0, len(task_files)-1)
        logging.info(f"Writing job file {fname}")
        file = open(fname,'w')
